...
class	Cactus : public Structure
{
	public:
		...
		std::vector<StructureBlock>	generate(WorldVec3i gen_pos)
		{
			...
			#define MIN_CACTUS_HEIGHT 2
			#define MAX_CACTUS_HEIGHT 20
			int	height_var = MIN_CACTUS_HEIGHT + static_cast<int>(rand3dTo1d(gen_pos) * (float)MAX_CACTUS_HEIGHT);
			...
			for (block_pos.y = gen_pos.y + 1; block_pos.y <= gen_pos.y + height_var; block_pos.y++)
				res.push_back({block_pos, BLOCK_CACTUS});
